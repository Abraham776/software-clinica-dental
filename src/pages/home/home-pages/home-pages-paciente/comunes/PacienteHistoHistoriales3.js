...
const HistorialOdontopediatria = () => {
	...
	const list = historial1.map(historialodontopediatria => {
		return (
			<tr>
				<td> {historialodontopediatria.FechaOdontopediatria} </td>
				<td> {historialodontopediatria.PronosticoOdontopedia} </td>
				<td><Button block onClick={function routePaciente() { window.location.href = `/ActuaHistoOdon/${historialodontopediatria.idHistorialOdontopediatria}` }}>Ingresar</Button></td>
				<td><Button block color="danger" onClick={function deletePaciente() {
					...
					dataService.delete(historialodontopediatria.idHistorialOdontopediatria)
						.then(response => {
							console.log(response.data);

							
							let index = historial1.map(dat => {
								return dat.idHistorialOdontopediatria === historialodontopediatria.idHistorialOdontopediatria
							})
	...
	return (
		<div className="home-contenido">
			...
			<div className="contenido-in-historiales">
				{/* Inputs */}
				<Form inline>
					<center><h1>Odontopediatria</h1></center>
					<br/>
					<h2>Paciente: <spam>Victor</spam></h2>
					<br/>
					   <Button onClick={function routeOrt(){window.location.href=`/NuevoHistorialOdon/${id}`}}>Agregar nueva historia clinica</Button>
					   <br/>  <br/>  <br/> 
