...
const HistorialOrtodoncia = () => {
	...
	const list = historial1.map(historialortod => {
		return (
			<tr>
				<td> {historialortod.FechaOrtod} </td>
				<td> {historialortod.PronosticoOrtod} </td>
				<td><Button block onClick={function routePaciente() { window.location.href = `/ActuaHistoOrt/${historialortod.idHistorialOrtod}` }}>Ingresar</Button></td>
				<td><Button block color="danger" onClick={function deletePaciente() {
					...
					dataService.delete(historialortod.idHistorialOrtod)
						.then(response => {
							console.log(response.data);

							
							let index = historial1.map(dat => {
								return dat.idHistorialOrtod === historialortod.idHistorialOrtod
							})
		...
		<div className="home-contenido">
			...
			<div className="contenido-in-historiales">
				...
				<Form inline>
					...
					<br/>
					   <Button onClick={function routeOrt(){window.location.href=`/NuevoHistorialOrt/${id}`}}>Agregar nueva historia clinica</Button>
					   <br/>  <br/>  <br/> 
...
export default HistorialOrtodoncia;